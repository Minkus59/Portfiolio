...
if ($Page=="All") {
    $SelectJpg = $cnx -> prepare("SELECT * FROM ".DB_PREFIX."Signature_Signer_Jpg WHERE page='1' AND hash=:hash");
    $SelectJpg-> BindParam(":hash", $_SESSION['hash2'], PDO::PARAM_STR);
    $SelectJpg-> execute(); 
    $AfficheJpg=$SelectJpg->fetch(PDO::FETCH_OBJ);

    echo "<img src='".$repExtJpgSigner.$AfficheJpg->fichier."' />";
}
elseif ($Page=="Last") {
    $SelectJpg2 = $cnx -> prepare("SELECT * FROM ".DB_PREFIX."Signature_Signer_Jpg WHERE hash=:hash");
    $SelectJpg2-> BindParam(":hash", $_SESSION['hash2'], PDO::PARAM_STR);
    $SelectJpg2-> execute(); 
    $AfficheJpg2=$SelectJpg2->fetch(PDO::FETCH_OBJ);

    $CountPage2=$SelectJpg2->rowCount();

    $SelectJpg = $cnx -> prepare("SELECT * FROM ".DB_PREFIX."Signature_Signer_Jpg WHERE page=:page AND hash=:hash");
    $SelectJpg-> BindParam(":hash", $_SESSION['hash2'], PDO::PARAM_STR);
...
elseif ($Action=="Date") { echo $Date; }
    ...
    $SelectFichier=$cnx->prepare("SELECT * FROM ".DB_PREFIX."Signature_Tampon WHERE id=:id");
