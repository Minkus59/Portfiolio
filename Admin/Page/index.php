...
$Select=$cnx->prepare("SELECT * FROM ".$Prefix."_Page WHERE sous_menu='0' ORDER BY position ASC");
$Select->execute();
...
while ($Page=$Select->fetch(PDO::FETCH_OBJ)) {
      $SelectSousMenu=$cnx->prepare("SELECT * FROM ".$Prefix."_Page WHERE parrin=:parrin AND sous_menu='1' ORDER BY position ASC");
      ...
      if ($CountSousMenu>0) {
            while ($SousMenu=$SelectSousMenu->fetch(PDO::FETCH_OBJ)) { 
                  $SelectSousSousMenu=$cnx->prepare("SELECT * FROM ".$Prefix."_Page WHERE parrin=:parrin AND sous_menu='1' ORDER BY position ASC");
