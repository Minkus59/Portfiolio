...
if ((!isset($_SESSION['StatueCompte']))||($_SESSION['StatueCompte']=="NULL")) {
  $Select=$cnx->prepare("SELECT * FROM ".$Prefix."_compte_Admin ORDER BY id DESC");
  $Select->execute();
}
else {
  $Select=$cnx->prepare("SELECT * FROM ".$Prefix."_compte_Admin WHERE valider=:valider ORDER BY id DESC");
