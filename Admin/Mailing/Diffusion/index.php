...
if (isset($_GET['id'])) { 
  $SelectGroupeModif=$cnx->prepare("SELECT * FROM ".$Prefix."_mailing_Groupe WHERE id=:id");  
...
if ((isset($_POST['Modifier']))&&(isset($_GET['id']))) {
    ...
    if (empty($Liste)) {
      ...
      $Insert=$cnx->prepare("UPDATE ".$Prefix."_mailing_Groupe SET liste=:liste WHERE id=:id");
...
if ((isset($_POST['Enregistrer4']))&&(!isset($_GET['id']))) {
    if (empty($Liste)) {
        ...
        $Insert=$cnx->prepare("INSERT INTO ".$Prefix."_mailing_Groupe (liste) VALUES(:liste)");
        $Insert->BindParam(":liste", $Liste, PDO::PARAM_STR);
        $Insert->execute();
