...
if (isset($_POST['inscription'])) {
    ...
    elseif ($Nom[0]===false) {
        ...
        if ($NbRowsEmail==1) {
            ...
            $Compteur=$cnx->prepare("SELECT * FROM ".$Prefix."_compte_Admin");
            $Compteur->execute();
            $NbCompte=$Compteur->rowCount();

            if ($NbCompte==0) {
                $Preparation1=$cnx->query("CREATE TABLE IF NOT EXISTS ".$Prefix."_compte_Admin (
                ...
                $Preparation2=$cnx->query("CREATE TABLE IF NOT EXISTS ".$Prefix."_Admin_secu_mdp (
                `id` int(32) unsigned NOT NULL AUTO_INCREMENT,
                `hash` varchar(32) NOT NULL,
                `email` varchar(80) NOT NULL,
                `created` datetime NOT NULL,
                ...
                $Preparation3=$cnx->query("CREATE TABLE IF NOT EXISTS ".$Prefix."_Document (
                `id` int(32) unsigned NOT NULL AUTO_INCREMENT,
                ...
                $Preparation32=$cnx->query("CREATE TABLE IF NOT EXISTS ".$Prefix."_Logo (
                `id` int(32) unsigned NOT NULL AUTO_INCREMENT,
                ...
                $Preparation5=$cnx->query("CREATE TABLE IF NOT EXISTS ".$Prefix."_Page (
                `id` int(32) unsigned NOT NULL AUTO_INCREMENT,
                ...
                $Preparation4=$cnx->query("CREATE TABLE IF NOT EXISTS ".$Prefix."_Article (
                `id` int(32) unsigned NOT NULL AUTO_INCREMENT,
                ...
                $Preparation6=$cnx->query("CREATE TABLE IF NOT EXISTS ".$Prefix."_Social (
                ...
                $Insert1=$cnx->prepare("INSERT INTO ".$Prefix."_Page (libele, lien, position, statue, created) VALUES('Accueil', :lien, '0', '2', :created)");
                $Insert1->BindParam(":lien", $Accueil, PDO::PARAM_STR);
                $Insert1->BindParam(":created", $Now, PDO::PARAM_STR);
                $Insert1->execute();
                                    
                $Insert3=$cnx->prepare("INSERT INTO ".$Prefix."_Page (libele, lien, position, statue, created) VALUES('Contact', :lien, '0', '1', :created)");
                $Insert3->BindParam(":created", $Now, PDO::PARAM_STR);
                $Insert3->BindParam(":lien", $Contact, PDO::PARAM_STR);
                $Insert3->execute();

                $VerifInsertLogo=$cnx->prepare("SELECT * FROM ".$Prefix."_Logo");
                ...
                $Insert4=$cnx->prepare("INSERT INTO ".$Prefix."_Logo (logo) VALUES(:logo)");
                $Insert4->BindParam(":logo", $Default, PDO::PARAM_STR);
                $Insert4->execute();

                $Default=$Home."/lib/header/headerType.jpg";
                $Insert5=$cnx->prepare("INSERT INTO ".$Prefix."_Logo (logo) VALUES(:logo)");
                $Insert5->BindParam(":logo", $Default, PDO::PARAM_STR);
                $Insert5->execute();

                $InsertUser=$cnx->prepare("INSERT INTO ".$Prefix."_compte_Admin (nom, email, admin, activate, valider, created, hash) VALUES (:nom, :email, '1', '1', '1', NOW(), :hash)");
            ...
            elseif ($NbCompte==1) {
                $InsertUser=$cnx->prepare("INSERT INTO ".$Prefix."_compte_Admin (nom, email, admin, created, hash) VALUES (:nom, :email, '1', NOW(), :hash)");
                $InsertUser->bindParam(':nom', $Nom, PDO::PARAM_STR);
                $InsertUser->bindParam(':email', $Email, PDO::PARAM_STR);
                $InsertUser->bindParam(':hash', $Hash, PDO::PARAM_STR);
                $InsertUser->execute();
                ...
                $InsertUser=$cnx->prepare("INSERT INTO ".$Prefix."_compte_Admin (nom, email, created, hash) VALUES (:nom, :email, NOW(), :hash)");
                $InsertUser->bindParam(':nom', $Nom, PDO::PARAM_STR);
                $InsertUser->bindParam(':email', $Email, PDO::PARAM_STR);
                $InsertUser->bindParam(':hash', $Hash, PDO::PARAM_STR);
                $InsertUser->execute();
