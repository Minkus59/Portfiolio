...
if ((!isset($_SESSION['StatueSocial']))||($_SESSION['StatueSocial']=="NULL")) {
     $Select=$cnx->prepare("SELECT * FROM ".$Prefix."_Social ORDER BY id ASC");
     $Select->execute();
}
else {
     $Select=$cnx->prepare("SELECT * FROM ".$Prefix."_Social WHERE statue=:statue ORDER BY id ASC");
     $Select->bindParam(':statue', $_SESSION['StatueSocial'], PDO::PARAM_STR);
     $Select->execute();
}

$SelectPage=$cnx->prepare("SELECT * FROM ".$Prefix."_Social");
$SelectPage->execute();
