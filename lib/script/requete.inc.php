...
try {
    ...
    $RecupArticle=$cnx->prepare("SELECT * FROM ".$Prefix."_Article WHERE page=:page AND statue='1' ORDER BY position ASC");
    ...
    $SelectPageActif=$cnx->prepare("SELECT * FROM ".$Prefix."_Page WHERE statue IN ('1', '2') AND sous_menu='0' ORDER BY position ASC");
    $SelectPageActif->execute();

    //Recup du menu footer actif
    $SelectPageActifFooter=$cnx->prepare("SELECT * FROM ".$Prefix."_Page WHERE statue IN ('1', '2') AND sous_menu='0' ORDER BY position");
    $SelectPageActifFooter->execute();

    //Recup du referencement
    $SelectPageSOE=$cnx->prepare("SELECT * FROM ".$Prefix."_Page WHERE lien=:page");
